    ...
    >
      <template #default>
        <el-form label-width="120px" :model="formData" ref="formRule" :rules="rules">
          <el-row>
            <el-col :span="12">
              <el-form-item label="板卡名称：" prop="name">
            ...
            <el-col :span="12">
              <el-form-item label="板卡ip：" prop="boardCardIp">
    ...
    onSure() {
      this.$refs.formRule.validate(valid => {
        if (valid) {
          ...
          this.isLinking = true;
          if (this.formData.id) {
            updateCardApi(this.formData).then(res => {
              if (res.code == 200) {
            ...
            addCardApi(this.formData).then(res => {
              if (res.code == 200) {
                this.$message.success("操作成功!");
                this.visible = false;
                this.$emit("getData");
